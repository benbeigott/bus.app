...
export default function FuelPriceTicker({ prices, isLive }: Props) {
  ...
  return (
    <div className="hidden lg:flex items-center gap-3 border border-yellow-500/15 bg-yellow-500/[0.04] rounded-xl px-4 py-2">
      <SkullPumpIcon />
      <div className="space-y-1">
        {diesel && (
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest w-14">Diesel</span>
            <span className="text-sm font-bold text-yellow-400 tabular-nums">
              {diesel.price.toFixed(3).replace(".", ",")} €
            </span>
            {diesel.change !== 0 && (
              <span className={`text-[10px] font-semibold ${diesel.change > 0 ? "text-red-400" : "text-green-400"}`}>
                {diesel.change > 0 ? "▲" : "▼"} {Math.abs(diesel.change).toFixed(3).replace(".", ",")}
              </span>
            )}
          </div>
        )}
        {e5 && (
          <div className="flex items-center gap-2">
            <span className="text-[10px] text-zinc-500 uppercase tracking-widest w-14">Super E5</span>
            <span className="text-sm font-bold text-yellow-400 tabular-nums">
              {e5.price.toFixed(3).replace(".", ",")} €
            </span>
            {e5.change !== 0 && (
              <span className={`text-[10px] font-semibold ${e5.change > 0 ? "text-red-400" : "text-green-400"}`}>
                {e5.change > 0 ? "▲" : "▼"} {Math.abs(e5.change).toFixed(3).replace(".", ",")}
              </span>
            )}
          </div>
        )}
        <div className="flex items-center gap-1.5 pt-0.5">
          <span className={`w-1.5 h-1.5 rounded-full ${isLive ? "bg-green-400" : "bg-zinc-600"}`} />
          <span className="text-[9px] text-zinc-600 uppercase tracking-widest">
            {isLive ? "Live · Ø Region" : "Offline"}
          </span>
        </div>
      </div>
    </div>
